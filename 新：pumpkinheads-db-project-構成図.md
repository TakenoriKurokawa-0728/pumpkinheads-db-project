...
--------------------------------------------------------------------------------
-- 6. 愛機・機材（Gears / Verification）セクタ
--------------------------------------------------------------------------------
CREATE TABLE public.m_gears (
    gear_id          SERIAL PRIMARY KEY,    -- 機材Root（10）
    member_id        INTEGER REFERENCES public.m_members(member_id),
    brand_name       VARCHAR(100) NOT NULL, -- ブランド（Gibson, ESP等）
    model_name       VARCHAR(200) NOT NULL, -- モデル名
    body_color       VARCHAR(50),
    is_main          BOOLEAN DEFAULT TRUE,  -- 魂の第一選択
    custom_specs     TEXT                   -- 改造セクタ（EMG, Floyd Rose等）
);

-- 機材検証アーカイブ（雑誌ごとの「真実」をマウント）
CREATE TABLE public.t_gear_verifications (
    verification_id  SERIAL PRIMARY KEY,
    gear_id          INTEGER REFERENCES public.m_gears(gear_id) ON DELETE CASCADE,
    magazine_id      INTEGER REFERENCES public.m_magazines(magazine_id),
    issue_date       DATE,                  -- 掲載号の発売日
    page_number      INTEGER,               -- 物理インデックス（ページ）
    comment_summary  TEXT                   -- ソースごとの検証要約
);

-- 加速装置：特定の機材の全検証履歴を1.8msで射抜く索引
CREATE INDEX idx_gear_verif_sync ON public.t_gear_verifications (gear_id);
